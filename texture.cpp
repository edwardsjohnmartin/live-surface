...
void LoadTexture(const string& filename, int texture_id, int* w, int* h) {
...
GLuint InitTexture(GLuint texture, unsigned char* data,
                   int width, int height, int channels, bool wrap, bool free_data) {
  typedef unsigned char BYTE;
  ...
  if (channels == 1) {
    glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RED, width, height, 0, GL_RED,
        GL_UNSIGNED_BYTE, data);
    // glGenerateMipmap(GL_TEXTURE_2D);  //Generate mipmaps now!!!
    gluBuild2DMipmaps(
        GL_TEXTURE_2D, GL_RED, width, height,
...
GLuint png_texture_load(
    const char * file_name, int * width, int * height,
